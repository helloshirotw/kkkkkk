...
class BaseViewController: UIViewController {
    ...
    func addSlideMenuButton(){
        let btnShowMenu = UIButton(type: UIButton.ButtonType.infoLight)
        btnShowMenu.setImage(self.defaultMenuImage(), for: UIControl.State())
        btnShowMenu.frame = CGRect(x: 0, y: 0, width: 30, height: 30)
        btnShowMenu.addTarget(self, action: #selector(BaseViewController.onSlideMenuButtonPressed(_:)), for: UIControl.Event.touchUpInside)
        let customBarItem = UIBarButtonItem(customView: btnShowMenu)
        self.navigationItem.leftBarButtonItem = customBarItem
//        let rightBarItem = UIBarButtonItem(title: "登入", style: .plain, target: self, action: #selector(loginButtonTapped))
//        self.navigationItem.rightBarButtonItem = rightBarItem
    ...
    func defaultMenuImage() -> UIImage {
        ...
        UIBezierPath(rect: CGRect(x: 0, y: 10, width: 30, height: 2)).fill()
        UIBezierPath(rect: CGRect(x: 0, y: 17, width: 30, height: 2)).fill()
